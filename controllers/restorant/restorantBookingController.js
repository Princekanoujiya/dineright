...
function getTimeDifferenceInMinutes(bookingDate, startTime, timeStamp) {
  ...
  // Parse the startTime string to hours and minutes
  const [startHours, startMinutes] = startTime.split(':').map(Number);

  // Ensure that the hours and minutes are valid numbers
...
exports.releaseTable = async (req, res) => {
  try {
    ...
    const [bookings] = await db.promise().query(bookingQuery, [booking_id]);
    ...
    // Update table status and end time
    const updateTableQuery = `UPDATE allocation_tables SET table_status = 'released', end_time = ?, slot_time = ? WHERE booking_id = ?`;
    await db.promise().query(updateTableQuery, [roundedTime, timeDifference, booking_id]);

    res.status(200).json({ message: 'Table released successfully', response: true });
  } catch (error) {
