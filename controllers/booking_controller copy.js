...
exports.book_product = async (req, res) => {
  ...
  // const dinigArea = await fetchDiningArea(userId, res);

  const data = fetchAllDiningAreas(res);
...
// function to fetch dining area
const fetchDiningArea = (userId, res) => {
  if (!userId) {
    return res.status(400).json({ error: "No userId provided for fetch dining area." });
  }

  // SQL query to fetch dining area data where userId matches
  const query = `
    SELECT * FROM selected_dining_areas WHERE userId = ?
  `;

  // Execute the query
  db.query(query, [userId], (err, result) => {
    if (err) {
      return res.status(500).json({ error: "Failed to fetch dining area data", details: err.message });
    }

    if (result.length === 0) {
      return res.status(404).json({ error: "No dining area data found for the provided userId." });
    }

    // Return the fetched dining area data
    return res.status(200).json(result);
  });
};

const fetchAllDiningAreas = (res) => {
  // SQL query to fetch all dining area data
  const query = `
    SELECT * FROM selected_dining_areas
  `;
  ...
  db.query(query, (err, result) => {
    ...
    if (result.length === 0) {
      return res.status(404).json({ error: "No dining area data found." });
    }

    // Return all the fetched dining area data
    return res.status(200).json(result);
  });
};
